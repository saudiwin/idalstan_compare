...
2. Stan scales well with large numbers of parameters, which is an attribute of most dimension reduction methods. Advances in Hamiltonian Monte Carlo estimation, including the use of variational inference and soon parallel computing within chains, promises to make full Bayesian inference of these models practical even with large datasets.
...
This ordered logit has to be embedded in the hurdle model to account for one-way censoring. Each legislator has a choice $r \in \{0,1\}$ in which $r=1$ if a legislator is absent and $r=0$ otherwise. With this notation, the likelihood of the hurdle model takes the following form:
...
```{r pos_discrim}
read_csv('data/high_coffee_discrim_coded.csv') %>% 
  mutate(Discrimination=value) %>% 
  select(Description, Discrimination) %>% 
  distinct %>% 
  DT::datatable()
```
...
```{r neg_discrim}
read_csv('data/low_coffee_discrim_coded.csv') %>% 
  mutate(Discrimination=value) %>% 
  select(Description, Discrimination) %>% 
  distinct %>% 
  DT::datatable()
...
```{r bill_plot}
id_plot(sen_est,person_ci_alpha=0.1,bill_plot=205,
...
`idealstan` is an effort to put state-of-the-art ideal point models along with the power of full Bayesian analysis in the hands of applied researchers in the social sciences. While the empirical examples presented were from the US Congress, the model can be used in any situation in which the assumptions of the ideal point model (unconstrained ideal point space) apply. While similar in function and form to the `edstan` package, the `idealstan` package has to use more complicated forms of identification because of the difficulty of leaving discrimination parameters unconstrained.
