...
format: 
  pdf: default
linestretch: 1.5
abstract: 'This paper presents a novel parameterization of ideal point models that addresses long-standing issues in the estimation of time-varying ideal points for diverse time series processes, sparse data, and missing data. In addition, the model employs Stan, a robust Markov Chain Monte Carlo sampler, to permit parallelization of Bayesian inference and to resolve issues of convergence for high-dimensional time-series. The model is compared to existing dynamic ideal point models using simulations to diagnose issues of bias and convergence and then to empirical data from the U.S. Congress from 1990 to 2018. Novel parameterizations in the model permit the estimation of monthly changes in Congressperson ideological trajectories while incorporating non-ignorable missing data. All models are implemented in the R package `idealstan`. WORD COUNT: 11,725'
...
This paper resolves long-standing issues in dynamic ideal point modeling within a novel Bayesian modeling framework. The goal is to make the ideal point model a robust tool for measuring latent concepts across the social sciences, applicable to time-varying, noisy, and potentially missing data from social media, surveys, legislatures, bureaucracies, or social networks. Implemented in Stan [@CarpenterGelmanHoffmanEtAl2017], a probabilistic programming language for Bayesian inference, this framework allows for flexible time-series processes, diverse data distributions, and novel missing data approaches. The `idealstan` R package implements these models with advances in Bayesian computation to handle larger datasets than was previously feasible with Markov Chain Monte Carlo methods.
...
The model's contributions are two-fold: 1) integrating novel time series processes and distributions into dynamic ideal point models, and 2) addressing computational challenges with big and sparse datasets. The main advantage of this framework is to enable researchers to estimate theoretically-relevant latent traits with robust Bayesian uncertainty. The approach further democratizes advanced measurement models, making them accessible to applied researchers without advanced training in Bayesian methods [@claassen2020].

Employing these methods, I estimate monthly ideal points for all legislators in the U.S. House from 1990 to 2018. This type of estimation is impossible with existing methods because sparsity undermines models' ability to converge. In addition, as I show with simulations, time-series processes are much more difficult to identify statistically, which can limit the applicability of existing models.

All models can be fit with `idealstan`, which employs Stan's Hamiltonian Monte Carlo algorithms for robust inference on challenging posterior geometries. By combining missing data adjustments, time-varying inference, and scalable computation, this framework provides a versatile tool for addressing complex time-varying measurement challenges across the social sciences.
...
Ideal point models have a long history in political science, and time-varying ideal point models have been available for almost as long. Latent measurements derived from these models, including DW-NOMINATE scores for the U.S. Congress and comparable measures for the U.N. General Assembly and human rights compliance, have made an outsize impact on research in the discipline. These uni-dimensional estimates of hard-to-measure traits permit researchers to test hypotheses with over-time variation to explore evolutionary changes. While over-time variation is no substitute for a causally-identified experiment, it can help rule out important inference issues like reverse causality and is necessary for methods like difference-in-differences.

For these reasons, the growth of the availability of big datasets with over-time variation, such as datasets derived from social media, digitized documents, historical coding exercise, or long-running surveys, offer important new areas of application of the ideal point model. However, it can be difficult to repurpose ideal point models to new time-varying domains as over-time variation poses significant hurdles. Many advances in ideal point modeling remain limited to static models, such as missing-data adjustment and diverse outcomes, while existing models have specific time-series processes that may not apply well to other types of phenomena.

The aim of this paper is to derive novel parameterizations of ideal point models to address these long-standing issues. I argue that a useful time-varying ideal point model should be able to handle diverse types of data, allow for diverse time series processes, and be robust to both computational complexity and high-dimensional inference. These are considerable challenges, but the model presented here makes progress on all of these fronts.
...
This type of parallelization of the gradient calculations is implemented for all of the models previously described. This allows for the computation of Bayesian ideal points for datasets that were previously intractable due to the length of time required. However, it should be noted that this type of parallelization does have significant overhead as not all of the computations of the Markov Chain are parallelizable, and leading to significant information that must be shared across parallel threads. Furthermore, this type of parallelization is only possible using in-memory parallel techniques, which restricts the application to single nodes on a computer cluster.
...
Having specified the types of models being proposed, I next turn to a new post-estimation quantity that helps clarify what ideal points substantively mean. Scholars want to identify latent traits because they want to understand what factors might predict the latent trait. Most analyses with latent variables analyze them as observed covariates in a regression model, potentially with the method of composition to propagate measurement uncertainty [@caugheySubstanceChangeCongressional2016].

However, it is also possible to jointly estimate the covariate as a hierarchical predictor in the model, as noted previously. While this type of analysis has been available for some time, the benefit of performing this kind of joint estimation, as opposed to using a summary measure in a downstream analysis, has not been made sufficiently clear, which is likely why it is rarely used in practice. Ideal point marginal effects make it clear what can be substantively learned by jointly estimating the external covariate with the latent trait. In particular, ideal point marginal effects can reveal the relationship between the covariate and the observed indicators while taking into account the mediation of the latent variable—that is, the effect of the covariate on outcomes that is weighted by the relevance of the outcomes to the latent variable.

To define this estimand, I return to the notation used previously and take the derivative of @eq-likem. For a given ideal point $\alpha_{it}$, the marginal effect (or first derivative) of an external covariate $x$ on an outcome $Y_{ijt}$ with a likelihood function $L_m(\cdot)$ is equivalent to:
...
What is important is that the quantity in @eq-deriv can be decomposed into three constituent parts: the derivative of the likelihood for the given model $L_m'(\cdot)$, the value of the discrimination parameter $\gamma_j$, and the value of the coefficient of the covariate, $\phi$. Multiplying these terms together will obtain the simultaneous change in a given item $Y_{ijtm}$ for a one-unit change in the covariate $x$.

From the notation it is clear that this formula permits the marginalization of marginal effects that aggregates across distinct likelihoods $M$. However, doing so could result in marginal effect estimates that are difficult to interpret as it would involve averaging over the different scales of the mixed outcome $Y_{ijtm}$, such as a marginal effect that is 40% the change in a count variable and 60% change in the probability of an ordinal outcome. For practical purposes, then, I assume that ideal point marginal effects should be reported separately for each distribution $m \in M$, and potentially each item $j \in J$, for a given mixed outcome.

There are two methods to calculate these types of marginal effects. The first would be to analytically calculate these quantities for each likelihood. While this is possible to do for some distributions, it is quite complicated as different linear model specifications, such as including a quadratic term for $x$, would necessitate a separate analytical derivative. A much more flexible method is to adopt numerical differentiation instead.
...
This sample-average quantity may be less interesting as it averages over item discrimination, which, as I show in the empirical section, is one of the most useful aspects of jointly estimating a covariate with the latent variable. An ideal point marginal effect of a covariate $x$ defined for persons $i$ and item $j$ is the change in that item given its relative polarity and the relative ideal point of the person. As I demonstrate in the empirical section, the disaggregated item-specific marginal effect intuitively captures the latent variable information encoded in the parameters in a way that makes the relationship clear between the ideal point covariate and the indicators for the latent trait.

The method of numerical differentiation enables the calculation of marginal effects from a variety of linear model specifications. For example, to calculate these marginal effects when including a squared term for $x$, we simply extend the formula by including this term in the limit,
