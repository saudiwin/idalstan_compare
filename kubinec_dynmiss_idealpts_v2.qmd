...
[^4]: Though the `emIRT` documentation says it uses multiple cores to speed up estimation, this does not seem to apply to its dynamic ideal point methods. Speed is essentially the same with or without multiple cores.
...
The simulation results are shown in @fig-rmse, @fig-kendall, @fig-serrs, @fig-merrs, and @fig-time, with additional results in SI section 5. @fig-rmse and @fig-kendall show the performance of each method in terms of capturing each individual ideal point relative to true estimates. @fig-rmse shows that `idealstan` (with HMC) had lower RMSE and a higher Kendall tau coefficient across time processes, as would be expected given that `idealstan` is the "true" model. Furthermore, the missing data adjustment has almost no penalty on these metrics, showing that `idealstan` with the HMC estimation method is able to fully endogenize this missingness and return accurate measures of ideal points.
...
The results for the downstream regression of estimated ideal point scores show similar trends with some noticeable differences. @fig-serrs reveals that the difference between models is more modest for the S error metric, although `idealstan` with the HMC method is still superior. There remains a noticeable difference between estimations with missing and non-missing data for the random walk, AR(1), and Gaussian process specifications, with methods like `mcmcPACK` resulting in S error rates that are about 50% greater on average than with observed data.

The M error plot in @fig-merrs, which aggregates across time series methods, shows that there is indeed a substantial penalty for alternative methods in terms of time series and missing data mis-specification. Interestingly, while the approximation methods Laplace and Pathfinder perform better on average than `emIRT` and `mcmcPACK`, they show high M error rates with missing data adjustment, while the `idealstan` method with HMC does not. As can be seen, the approximation methods seem to perform better with simpler ideal point specifications, though they can still out-perform a robust method like MCMC with the wrong model.

Finally, @fig-time shows that `emIRT` remains the fastest estimation method as a function of the number of items included in the simulation, followed by Pathfinder and then the Laplace approximation. `idealstan` and `mcmcPACK` are close in estimation time with a smaller number of items (100), but diverge significantly as the number of items increases. `idealstan` HMC with parallelization of 4 cores is approximately $O(N)$ while `mcmcPACK` and its Gibbs sampler is approximately $O(N^2)$, which makes larger models intractable. As the data size increases, the parallel overhead remains constant, which makes the `idealstan` within-chain parallelization scheme more efficient. In addition, speed can be improved for `idealstan` by increasing the number of cores up to the number of persons in the data. The level of speed enhancement for an increasing number of cores up to 48 and fixed data with `idelastan` is shown in the SI in Figure 2.
