...
This paper shows why measurement models need to become more robust and flexible in order to handle the difficult inference challenges of contemporary social science scholarship with noisy dynamic data. To accomplish this task, I define a novel parameterization of the ideal point model that is implemented in Stan [@CarpenterGelmanHoffmanEtAl2017], a probabilistic programming language for Bayesian inference. This new framework enables the researcher to select a parameterization for measurement estimation that most closely matches the theoretical nature of the construct of interest while enabling direct comparisons to alternative parameterizations of the latent trait. In addition, the `idealstan` R package implements these new methods with advances in Bayesian computation to handle datasets that are several orders of magnitude larger than was previously feasible with exact Bayesian inference while also adjusting for non-ignorable missing data.

Initially designed for measuring U.S. Congressional ideology, the ideal point model has proven valuable for various social science measurement challenges, including Twitter data [@barbera2015; @kubinecWhenGroupsFall2021; @eadyNewsSharingSocial2025], state capacity [@hansonLeviathansLatentDimensions2021], judiciaries [@ovádek2021], lobbying [@kim2017], the level of democracy [@coppedgeMethodologyVarietiesDemocracy2019; @pemsteinDemocraticCompromiseLatent2017; @grumbachLaboratoriesDemocracyHow2022], and large language model output [@burnham], to name only a few. Its utility lies in measuring latent traits where data are imperfect proxies of theoretically defined concepts. However, while social scientists increasingly have access to granular time-varying data, dynamic ideal point modeling is limited to measures of binary outcomes, very specific time-series processes, fully observed data, and depending on the estimation framework, datasets of small to medium size. As a result, scholars often must choose a measurement model with assumptions that are inappropriate for the measurement problem at hand, which can perversely lead to new measurement error via model mis-specification. On the other hand, if scholars choose to ignore measurement estimation altogether, they can end up with fragile inferences that depend on arbitrary measurement artifacts [@ranehillAssessingRobustnessPower2015; @jerzak2024].
...
The `idealstan` model achieves a higher level of flexibility and robustness by fully exploiting computational and statistical advances in Bayesian modeling. `idealstan` is built on top of Stan, which is a powerful sampler for Bayesian inference using Hamiltonian Markov Chain Monte Carlo (HMC) [@betancourt2013]. Relative to existing Bayesian frameworks, such as Metropolis Hastings or Gibbs samplers, HMC is designed for high-dimensional inference (i.e., lots of correlated parameters), and with an auto-differentiation library, can incorporate most statistical distributions for parameters without requiring conjugacy (i.e., mathematical tractability of the posterior distribution). As a consequence, at least in theory, Stan can fit virtually any joint posterior distribution for parameters that is a proper statistical distribution.

For these reasons, Stan-based measurement models have already made a significant impact in the discipline [@barbera2015; @caugheyDynamicEstimationLatent2015; @coppedgeMethodologyVarietiesDemocracy2019]. The main limitation of existing work is that tweaking any of these existing Stan-based models results in a new model, which must be thoroughly tested for convergence. For these reasons, the approach taken in this paper is to fully exploit Stan's flexibility by designing a meta-measurement model that can capture a wide variety of latent traits, which enables scholars to choose the most theoretically-appropriate specification instead of shoehorning their data into a specification designed for a different concept.
...
In this paper, and in the definitions that follow, I focus on those contributions of `idealstan` that advance the research agenda of dynamic measurement design. However, it is important to note that `idealstan`'s modular architecture, built on top of Stan's state-of-the-art Bayesian sampling system [@CarpenterGelmanHoffmanEtAl2017], can be further applied to statistical measurement problems beyond those discussed in this paper, some of which is discussed in the supplementary information.
...
We then need obtain posterior inference on this set of unobserved parameters $\{\alpha_{it},\gamma_j,\beta_j\} \in \theta$ that is conditional on the observed measurements $Y_{ijt}$, or $Pr(\theta|Y_{ijt})$. To do so, I define independent priors on the set of parameters $\theta$, $Pr(\theta)$, which is multiplied by an appropriate likelihood $L(\cdot)$ for the measures $Y_{ijt}$ as shown in @eq-Bayes:[^1]
...
$$
Pr(\theta|Y_{ijt}) \propto Pr(\theta)L(Y_{ijt}|\theta)
$$ {#eq-Bayes}
...
\begin{align}
X_1 &\sim \text{Normal}(1 + 0.5\alpha_{it},1)\\
Y &\sim \text{Normal}(-2 + \beta_S\alpha_{it}X_1 + 3X_1,1)
\end{align}
...
The aim of the simulation is to compare the different models by how well they can estimate the true (standardized) ideal points, measured with RMSE and Kendall's tau rank coefficient for rank order, along with inference on downstream quantities ($\beta_S$). I measure downstream inference performance using the S error and M error statistics in which an S error occurs when an estimate of $\beta_S$ is statistically significant at the 0.05 level but of the wrong sign and an M error is the relative inflation of statistically significant estimates to the true values (i.e., the absolute value of the ratio) [@gelman2014types]. I employ these measures, as opposed to predictive measures of model fit, as they correspond more closely to the goals scholars aim to achieve with ideal point models: measuring the latent trait accurately for a set of persons or institutions and using estimated scores as covariates in downstream analyses.[^5]

[^5]: Some scholars have been employing the method of composition to propagate measurement errors in downstream models [@jerzak2024], but lately this method has come under criticism for lacking a clear statistical basis. For these reasons I do not perform this additional adjustment as error propagation is not particularly important for the task of comparing across models.
...
One caveat I must mention is that it is difficult to compare latent variable estimation methods as relatively trivial differences can manifest themselves in significant changes in the scale of a latent variable. Employing standardization is helpful as it removes some of these scaling differences, though it can also introduce bias as well because the transformation is nonlinear. Kendall's tau rank coefficient also helps to alleviate these issues by comparing ranks of ideal points, which should be unaffected by any monotone transformation. All models are given the same amount of identification information (persons or item parameters), and in addition, if a model converges to the wrong mode (i.e., the sign of the ideal points is flipped), the ideal points are sign-reversed before calculating relevant statistics to avoid obfuscating model fit with arbitrariness in identification conditions.
...
The simulation results are shown in  @fig-kendall and @fig-time, with supplementary results in SI Figures 7 (RMSE), 8 (S-errors), and 9 (M-errors) (see SI Section 5). @fig-kendall and SI Figure 7 show the performance of each method in terms of capturing each individual ideal point relative to true estimates. @fig-kendall shows that `idealstan` (with HMC) had a higher Kendall tau coefficient (correlation) across time processes, as would be expected given that `idealstan` is the "true" model. Furthermore, the missing data adjustment has almost no penalty on these metrics, showing that `idealstan` with the HMC estimation method is able to fully endogenize this missingness and return accurate measures of ideal points. As can be seen, there are considerable penalties for estimation accuracy when either the time process is mis-specified, missing data is ignored, or both in combination.
...
While this result was expected given that `idealstan` is the true model, it is more interesting to compare the performance of the approximation methods along with the other models. `emIRT` and `mcmcPACK` show significant declines in performance when missing data is included in both RMSE (SI Figure 7) and Kendall tau coefficients (@fig-kendall), though the difference is more noticeable in the Kendall tau coefficients. The approximation methods Laplace and Pathfinder were more robust than the methods without missing data adjustments, although they performed better in simpler time series processes like splines and random walks. On the other hand, a noticeable gap exists for these metrics for approximations versus true HMC in the Gaussian process results.

`mcmcPACK`, which is the other Bayesian method under consideration, performed best in the random walk specification, which would be expected as that is the same time series process it employs. `mcmcPACK` performed as well as `emIRT` on the Gaussian process and AR(1) specifications, but performed much more poorly on the spline method. In fact, `emIRT` performed quite well on the spline process, with its RMSE score better than the Pathfinder and Laplace approximation methods even though it employs a random walk time series method. This exceptional performance might be a side effect of the variational method in `emIRT` that simplifies the random walk process into a lower-dimensional space [@imaiFastEstimationIdeal2016] and potentially fits a spline function better.

The results for the downstream regression of estimated ideal point scores show similar trends with some noticeable differences. SI Figure 8 reveals that the difference between models is more modest for the S error metric, although `idealstan` with the HMC method is still superior. There remains a noticeable difference between estimations with missing and non-missing data for the random walk, AR(1), and Gaussian process specifications, with methods like `mcmcPACK` resulting in S error rates that are about 50% greater on average than with observed data.

The M error plot in SI Figure 9, which aggregates across time series methods, shows that there is indeed a substantial penalty for alternative methods in terms of time series and missing data mis-specification. Interestingly, while the approximation methods Laplace and Pathfinder perform better on average than `emIRT` and `mcmcPACK`, they show high M error rates with missing data adjustment, while the `idealstan` method with HMC does not. As can be seen, the approximation methods seem to perform better with simpler ideal point specifications, though they can still out-perform a robust method like MCMC with the wrong model.

Finally, @fig-time shows that `emIRT` remains the fastest estimation method as a function of the number of items included in the simulation, followed by `idealstan` with the Pathfinder and Laplace approximations. `idealstan` with HMC and `mcmcPACK` are close to each other in estimation time when the number of items is small (100), but diverge significantly as the number of items increases. Estimation time for `idealstan` with parallelization of 4 cores is approximately linear in the number of items $N$ ($O(N)$) while `mcmcPACK` and its Gibbs sampler is approximately quadratic in the number of items ($O(N^2)$), which makes larger datasets intractable for `mcmcPACK`.

The reason for the superior performance of `idealstan` with a larger number of items is that the parallel overhead remains relatively constant, which makes the `idealstan` within-chain parallelization scheme more efficient as datasets increase in size. In addition, speed can be improved for `idealstan` by increasing the number of cores up to the number of persons in the data. The level of speed enhancement for an increasing number of cores up to 48 and fixed data with `idelastan` is shown in SI Figure 2.
...
In this section, I combine the missing-data adjustment along with the new time-series methods in `idealstan` to show how to connect modeling approaches to theories. Unlike existing methods, I do not simply adopt a single time series process, but rather explore the posterior trajectory of different time series processes to determine which process most closely fits the latent concept of interest, which is in this case the canonical application of legislator ideology.

To do so, I fit time-varying ideal point models to estimate monthly legislator ideal points for Congresspeople in the U.S. House–a much finer resolution than the yearly dynamic ideal points that have been used for some time [@poole1997]. While my total dataset encompasses the period 1990 to 2018, I focus first on the 115th Congress (2016 to 2018) as it permits easier comparison of the different time series methods.
...
To preview the findings, I show that the simpler time series models, i.e. the splines, are the best option for higher resolution ideal points for this type of legislative behavior. The other more complicated time-series models are only identified because of the use of Pathfinder to identify starting values in a single mode. While this is a valid form of measurement inference, as I show below, these more flexible models are clearly not appropriate for a theoretical construct that varies only slowly at the monthly level. The issue is not technical but rather theoretical: time-series specifications with large numbers of parameters per legislator allow more over-time change in ideal points than the theoretical construct (ideology) permits.
