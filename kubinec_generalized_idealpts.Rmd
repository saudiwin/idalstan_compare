...
if(cluster) {
  ...
  saveRDS(unemp3_fit,'data/unemp3_fit.rds')
  ...
  if(cluster) {
    ...
    unemp1_fit <- readRDS('/scratch/rmk7/unemp1_fit.rds')
    unemp2_fit <- readRDS('/scratch/rmk7/unemp2_fit.rds')
    unemp3_fit <- readRDS('/scratch/rmk7/unemp3_fit.rds')
...
if(run_all) {
  rm(unemp1)
rollcalls <- readRDS('data/rollcalls.rds') %>% 
  select(cast_code,rollnumber,year,congress,
         bioname,party_code,date,unemp_rate,
...
rollcalls <- filter(rollcalls,!is.na(x)) %>% 
  mutate(item_id=paste0(congress,"_",rollnumber))
