...
abstract: 'The increasing importance paid to measurement in political science makes the ideal point model a helpful framework for estimating latent concepts with diverse data. This  paper presents an item-response theory parameterization of ideal points that unifies existing approaches to ideal point models while also extending them in a unified framework. For time-varying inference, the model permits ideal points to vary in a random walk, in a stationary autoregressive process, or in a semi-parametric Gaussian process. For missing data, the model implements a two-stage selection adjustment to account for non-ignorable missingness. In addition, the ideal point model is extended to handle diverse outcomes, including ordinal, count, continuous, and positive-continuous distributions. To address limitations in estimating very large datasets, variational Bayesian inference is included as an alternative to full Markov Chain Monte Carlo inference.'
...
Up to this point, I have followed convention in assuming that the observed data $Y_{ij}$ is a binary outcome. Virtually any distribution for $Y_{ij}$ is possible, especially with the flexibility of Bayesian modeling. In this section I incorporate existing distributions used for ideal points while also introducing the reader to distributions, especially ordinal models, that have been rarely employed for this purpose but could have considerable utility. 
