...
format: pdf
linestretch: 1.5
abstract: 'This  paper presents an item-response theory parameterization of ideal points that employs advances in Bayesian computation to permit more flexible distributions, missing data mechanisms, quantities of interest and time-series processes. For missing data, the model implements a two-stage selection adjustment to account for non-ignorable missingness. The model includes  a variety of time-series processes, such as a random walk, AR(1), Gaussian processes, and splines. To better understand how external covariates affect ideal points, I put forward a new item-level quantity of interest, ideal point marginal effects, that are defined as the percent change in supporting an item (bill) for a one-unit change in a person-specific covariate. I demonstrate the utility of the package with a study of monthly district-level unemployment and voting behavior in the 115th Congress of the United States. All of these models are available via the R package `idealstan` with Hamiltonian Markov Chain Monte Carlo in Stan.'
...
It is important to note that there is nothing about this model that necessarily implies that the latent scale is political ideology. That is a useful interpretation to assign ideal points based on data from the U.S. Congress [@rosenthal2007]. The ideal point model represents a social choice process where the construct by which people make choices is unobserved. As such, it *can* estimate ideology as a latent construct, but it does not need to. Interpreting what the latent variable represents is a necessary part of any application of the model. What can be said is that it always represents the aggregation of a social choice process with competing alternatives.
...
In addition, we might think that any polarization may be assymetric in terms of different parties. In the 115th Congress, the Republicans held a majority with a Republican president in office. As a result, their response to unemployment shocks could vary from Democrats who were in the opposition. This leads us to a second hypothesis:
...
unemp1_fit <- readRDS('data/unemp1152_12_1_fit.rds')
unemp2_fit <- readRDS('data/unemp1152_12_2_fit.rds')
unemp3_fit <- readRDS('data/unemp1152_12_3_fit.rds')

# unemp1_all_fit <- readRDS('data/unempall2_12_1_fit.rds')
# unemp2_all_fit <- readRDS('data/unempall_12_2_fit.rds')
# unemp3_all_fit <- readRDS('data/unempall_12_3_fit.rds')

unemp_gp_fit <- readRDS('data/unemp115_2_12__gp_fit.rds')
unemp_ar_fit <- readRDS("data/unemp115_2_12_1_ar_fit.rds")
unemp_rw_fit <- readRDS("data/unemp115_2_12_1_rw_fit.rds")
...
saveRDS(rhatrw, "data/rhatrw.rds")
  ...
  rhatar <- readRDS("data/rhatar.rds")
  rhatrw <- readRDS("data/rhatrw.rds")
  rhatgp <- readRDS("data/rhatgp.rds")
...
# d2 <- unemp2_fit@stan_samples$draws("lp__") %>% apply(2,mean)
# d3 <- unemp3_fit@stan_samples$draws("lp__") %>% apply(2,mean)
# dgp <- unemp_gp_fit@stan_samples$draws("lp__") %>% apply(2,mean)
# dar <- unemp_ar_fit@stan_samples$draws("lp__") %>% apply(2,mean)
# drw <- unemp_rw_fit@stan_samples$draws("lp__") %>% apply(2,mean)
# 
# m2 <- which(d2==max(d2))
# m3 <- which(d3==max(d3))
# mgp <- which(dgp==max(dgp))
# mar <- which(dar==max(dar))
# mrw <- which(drw==max(drw))
...
rw_a <- id_plot_legis_dyn(unemp_rw_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
  ...
  un2_a <- readRDS("data/un1_a.rds")
  un3_a <- readRDS("data/un1_a.rds")
...
if(run_all) {
  ...
  un1_a1 <- id_plot_legis_dyn(unemp1_fit,include="AMASH, Justin",
                         use_ci=T,plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("Spline of 2nd Degree") + labs(y="") +
  scale_x_date(guide = guide_axis(n.dodge = 2)) + 
    scale_y_reverse()
  
  saveRDS(un1_a1, "data/un1_a1.rds")

un2_a1 <- id_plot_legis_dyn(unemp2_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("Spline of 3rd Degree") + labs(y="") +
  scale_x_date(guide = guide_axis(n.dodge = 2)) +
  scale_y_reverse()
 
  saveRDS(un2_a1, "data/un1_a1.rds")

un3_a1 <- id_plot_legis_dyn(unemp3_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("Spline of 4th Degree") + labs(y="") +
  scale_x_date(guide = guide_axis(n.dodge = 2))
  
  saveRDS(un3_a1, "data/un3_a1.rds")

gp_a1 <- id_plot_legis_dyn(unemp_gp_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("Gaussian Process") + labs(y="") +
  scale_x_date(guide = guide_axis(n.dodge = 2))

  saveRDS(gp_a1, "data/gp_a1.rds")

ar_a1 <- id_plot_legis_dyn(unemp_ar_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("AR(1) Process") + labs(y="") +
  scale_x_date(guide = guide_axis(n.dodge = 2))

  saveRDS(ar_a1, "data/ar_a1.rds")

rw_a1 <- id_plot_legis_dyn(unemp_rw_fit,use_ci=T,include="AMASH, Justin",plot_text = F) + scale_color_manual(values=c(R="red",
                              D="blue",I="green"),name="") + ggtitle("Random Walk") +
  scale_x_date(guide = guide_axis(n.dodge = 2)) + labs(y="") +
  scale_y_reverse()

  saveRDS(rw_a1, "data/rw_a1.rds")
  
} else {
  
  un1_a1 <- readRDS("data/un1_a1.rds")
  un2_a1 <- readRDS("data/un1_a1.rds")
  un3_a1 <- readRDS("data/un1_a1.rds")
  gp_a1 <- readRDS("data/gp_a1.rds")
  rw_a1 <- readRDS("data/rw_a1.rds")
  ar_a1 <- readRDS("data/ar_a1.rds")
...
un1_a1 + un2_a1 + un3_a1 + ar_a1 + gp_a1 + rw_a1 + plot_layout(nrow=2,
                                             guides="collect") + 
  plot_annotation(caption="Posterior average values shown for 115th Congress legislator ideal points.\nItem discrimination for Republican party-line votes was constrained to be positive.") & theme(legend.position = "top",
                                                                                                                                                                                                    plot.title=element_text(size=9))
...
bind_rows(s1_cov, s2_cov, s3_cov, sgp_cov, sar_cov, srw_cov) %>% 
