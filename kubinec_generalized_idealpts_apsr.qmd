...
This paper generalizes the ideal point model from political science to address long-standing issues like non-ignorable missing data while extending its domain to cover diverse distributions and time processes. The goal is to make the ideal point model a robust tool for measuring latent concepts across the social sciences, applicable to data from social media, surveys, legislatures, bureaucracies, or social networks. Implemented in Stan [@CarpenterGelmanHoffmanEtAl2017], a probabilistic programming language for Bayesian inference, this framework allows for flexible time-series processes, diverse data distributions, and novel missing data approaches. The `idealstan` R package implements these models with advances in Bayesian computation to handle larger datasets than was previously feasible with Markov Chain Monte Carlo methods.

Initially designed for measuring U.S. Congressional ideology, the ideal point model has proven valuable for various social science measurement challenges, including Twitter data [@barbera2015; @kubinec2018], the UN Security Council [@baileyEstimatingDynamicState2017], and the level of democracy [@coppedgeMethodologyVarietiesDemocracy2019; @pemsteinDemocraticCompromiseLatent2017], to name only a few. Its utility lies in measuring latent traits where data are imperfect proxies of theoretically defined concepts. To enhance its generalizability, this paper extends the model to address challenges like non-ignorable missing data [@rosasNoNewsNews2015], mixed outcome distributions [@imaiFastEstimationIdeal2016], and time-varying data [@caugheyDynamicEstimationLatent2015], as well as incorporating external covariates [@jackman2004].

The model's contributions are two-fold: 1) integrating diverse existing and novel parameterizations into a unified joint estimation framework, and 2) addressing computational challenges with big datasets. This framework enables researchers to estimate theoretically-relevant latent traits with robust Bayesian uncertainty. The approach further democratizes advanced measurement models, making them accessible to applied researchers without advanced training in Bayesian methods [@claassen2020].

Novel contributions include solutions for sparse time-series data using splines and new estimands like ideal point marginal effects, which reveal how external covariates influence observed indicators through the latent variable. Employing these methods, I estimate monthly ideal points for all legislators in the U.S. House from 1990 to 2018. Employing ideal point marginal effects, I also show that high district-level unemployment led House Democrats to moderate their positions, while Republicans remained unaffected. Finally, the empirical analysis reveals how the missing data adjustment can remove significant bias in both surveys and legislative data.

All models can be fit with `idealstan`, which employs Stan's HMC algorithms for robust inference on challenging posterior geometries. By combining missing data adjustments, time-varying inference, and scalable computation, this framework provides a versatile tool for addressing complex measurement challenges across the social sciences.
