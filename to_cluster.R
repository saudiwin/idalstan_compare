...
if(create_data) {
  ...
  # countun <- read_delim('/lustre/scratch/x-rkubinec/la_county.txt',delim="\t")
  #
  # countun <- mutate(countun,series_id=trimws(series_id)) %>%
  #   filter(period!="M13")
  # saveRDS(countun,'/lustre/scratch/x-rkubinec/countun.rds')
  
  # countun <- readRDS('/lustre/scratch/x-rkubinec/countun.rds')
  
  # # load series indicators
  # id_data <- read_tsv('/lustre/scratch/x-rkubinec/la_series.txt')
  #
  ...
  #   distinct
  #
  # saveRDS(bls_fips,'/lustre/scratch/x-rkubinec/bls_fips.rds')
  
  # bls_fips <- readRDS('/lustre/scratch/x-rkubinec/bls_fips.rds')
  #
  ...
  # saveRDS(county_series,'/lustre/scratch/x-rkubinec/county_series.rds')
  
  # county_series <- readRDS('/lustre/scratch/x-rkubinec/county_series.rds')
  ...
  # districts2018 <- st_read('/lustre/scratch/x-rkubinec/congress_shape/districtShapes/districts114.shp') %>% st_transform(albers)
  # districts2008 <- st_read('/lustre/scratch/x-rkubinec/congress_shape/districtShapes/districts110.shp') %>% st_transform(albers)
  # districts1998 <- st_read('/lustre/scratch/x-rkubinec/congress_shape/districtShapes/districts105.shp') %>% st_transform(albers)
  # districts1988 <- st_read('/lustre/scratch/x-rkubinec/congress_shape/districtShapes/districts100.shp') %>% st_transform(albers)
  #
  ...
  # })
  # saveRDS(dist_list,'/lustre/scratch/x-rkubinec/dist_list.rds')
  
  # county_space <- tigris::counties() %>%
  #   st_as_sf
  # saveRDS(county_space,'/lustre/scratch/x-rkubinec/county_space.rds')
  ...
  #
  #
  # county_space <- readRDS('/lustre/scratch/x-rkubinec/county_space.rds') %>%
  #               st_transform(albers) %>%
  #             st_buffer(dist = 0) %>%
  #   select(STATEFP,COUNTYFP,GEOID,geometry)
  #
  # dist_list <- readRDS('/lustre/scratch/x-rkubinec/dist_list.rds') %>%
  ...
  #     saveRDS(paste0('/lustre/scratch/x-rkubinec/',d,'_int.rds'))
  # })
  #
  # all_ints_names <- rev(list.files(path = "/lustre/scratch/x-rkubinec/",pattern="int.rds",full.names = T))
  #
  ...
  # dist_state <- readRDS('/lustre/scratch/x-rkubinec/dist_list.rds') %>%
  ...
  # over_states %>%
  #   filter(DISTRICT!='98') %>%
  #   saveRDS('/lustre/scratch/x-rkubinec/over_states.rds')
}
...
# 
# rollinfo <- read_csv('/lustre/scratch/x-rkubinec/Hall_rollcalls.csv')
# 
...
# # member votes
# 
# rollcalls <- read_csv('/lustre/scratch/x-rkubinec/Hall_votes.csv') %>% 
#   filter(congress>100)
# 
...
# # need member info
# 
# meminfo <- read_csv('/lustre/scratch/x-rkubinec/Hall_members.csv')
# 
...
# saveRDS(rollcalls,'/lustre/scratch/x-rkubinec/rollcalls.rds')
...
collapse_restrict <- group_by(check_bills1,congress) %>% 
  summarize(restrict_ind_high=c(min(rollnumber[polarity==1]),
                                ifelse(all(congress!=max_congress), 
                                       rollnumber[abs(rollnumber - quantile(rollnumber[polarity==1],.33)) == min(abs(rollnumber - quantile(rollnumber[polarity==1],.33)))],
                                       rollnumber[abs(rollnumber - median(rollnumber[polarity==1])) == min(abs(rollnumber - median(rollnumber[polarity==1])))]),
                                ifelse(all(congress!=max_congress), 
...
unemp1 <- unemp1  %>% 
  ...
  if(fit_type=="spline1") {
    ...
    saveRDS(unemp1_fit,paste0("/lustre/scratch/x-rkubinec/",modtype,is_missing,"_",max_treedepth,"_","1_fit.rds"))
  ...
  if(fit_type=="spline2") {
    ...
    saveRDS(unemp2_fit,paste0("/lustre/scratch/x-rkubinec/unemp",modtype,is_missing,"_",max_treedepth,"_","2_fit.rds"))
  ...
  if(fit_type=="spline3") {
    ...
    saveRDS( unemp3_fit,paste0("/lustre/scratch/x-rkubinec/unemp",modtype,is_missing,"_",max_treedepth,"_","3_fit.rds"))
...
if(fit_type=="GP") {
  ...
  saveRDS(unemp_gp_fit, paste0("/lustre/scratch/x-rkubinec/unemp",modtype,"_",is_missing,"_",max_treedepth,"_","_gp_fit.rds"))
...
if(fit_type=="ar1") {
  ...
  saveRDS(unemp1_ar_fit, paste0("/lustre/scratch/x-rkubinec/unemp",modtype,"_",is_missing,"_",max_treedepth,"_","1_ar_fit.rds"))
...
if(fit_type=="rw") {
  ...
  saveRDS(unemp1_rw_fit, paste0("/lustre/scratch/x-rkubinec/unemp",modtype,"_",is_missing,"_",max_treedepth,"_","1_rw_fit.rds"))
...
if(fit_type=="china") {
  ...
  rollcalls <- rollcalls %>% 
    ...
    mutate(decade=case_when(year<2000~1L,
                            ...
                            TRUE~3L)) %>% 
    filter(decade<3)
  load('/lustre/scratch/x-rkubinec/andy_hall_jop/fh_final_analysis.rdata')
  ...
  saveRDS(china_fit1,paste0('/lustre/scratch/x-rkubinec/china_',modtype,"_",is_missing,"_",max_treedepth,"_",'_fit1.rds'))
...
## 
## #unemp1_fit <- readRDS("/lustre/scratch/x-rkubinec/unemp1_run1fit.rds")
## 
